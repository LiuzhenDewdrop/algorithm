...
//			CodeEditor.setValue(decodeURI($('#answer').val()));
	...
	function submitForm(index) {
		if (!index || confirm("提交后将不可更改代码并计分, 是否确认提交？")) {
			$('#status').val(index);
			$('#code').val(CodeEditor.getValue());
			$.ajax({
				type:"POST",
				...
				success:function(data){
					alert(data);
					$('#answer').val(CodeEditor.getValue());
				},
				error:function () {
					alert("提交失败！");
				}
	...
	<div id="content" class="content bodyWidth m-auto">
		...
		<form name="codeForm" id="codeForm" method="post" action="">
			...
			<div class="testRunAction">
				...
				<button type="button" class="btn btn-danger m-r-5 m-b-5" onclick="submitForm(0)">暂 存</button>
				<button type="button" class="btn btn-danger m-r-5 m-b-5" onclick="submitForm(1)">交 卷</button>
